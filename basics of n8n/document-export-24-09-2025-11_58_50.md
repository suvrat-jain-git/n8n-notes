...
2. **On app event or Integration triggers** - They work when something new happens in the app.
3. **Scheduled trigger** - This triggers the workflow on a set schedule
4. **Webhook trigger** - Once active it listens for HTTP request. As a HTTP alert (along with data) is received it starts the workflow. Generally used when app trigger not available.
